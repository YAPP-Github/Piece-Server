...
public interface ProfileValuePickRepository extends JpaRepository<ProfileValuePick, Long> {
    ...
    @Query("SELECT p FROM ProfileValuePick p WHERE p.profile.id = :profileId and p.valuePick.isActive = true ORDER BY p.valuePick.id ASC")
    List<ProfileValuePick> findActiveProfileValuePickByProfileIdOrderByValuePickId(
